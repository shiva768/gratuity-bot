...
@Component("add-command-enable-channel")
@Text
@Slf4j
public class AddCommandEnableChannel implements MessageCommand<MessageReceivedEvent> {
    ...
    @Autowired
    public AddCommandEnableChannel(DynamicSettingService dynamicSettingService, MessageSourceWrapper messageSourceWrapper, DiscordValidatorService discordValidatorService) {
    ...
    public void execute(MessageReceivedEventAdapter eventAdapter, Locale locale) {
        ...
        try {
            ...
            if (!validate(eventAdapter, locale, discordObject))
                return;
            dynamicSettingService.save(DynamicSettingType.CommandEnableChannel.getKey(), discordObject.getConvertedId(), eventAdapter.getGuildId(), discordObject.getConvertedId());
