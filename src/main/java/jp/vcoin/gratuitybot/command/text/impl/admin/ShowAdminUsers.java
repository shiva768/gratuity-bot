...
@Component("show-admin-users")
@Text
@Slf4j
public class ShowAdminUsers implements MessageCommand<MessageReceivedEvent>, ShowDynamicSettingCommand {
    private final DynamicSettingService dynamicSettingService;
    private final ApplicationProperties applicationProperties;
    private final MessageSourceWrapper messageSourceWrapper;

    @Autowired
    public ShowAdminUsers(DynamicSettingService dynamicSettingService, ApplicationProperties applicationProperties, MessageSourceWrapper messageSourceWrapper) {
    ...
    public void buildContent(MessageReceivedEventAdapter eventAdapter, Locale locale, EmbedBuilder builder) {
        final List<AdminUser> adminUsers = getDynamicSettingDomains(eventAdapter.getGuildId());
        adminUsers.forEach(d -> builder.appendField(
                messageSourceWrapper.getMessage(DynamicSettingDomain.SETTING_NAME_KEY_PREFIX + DynamicSettingType.AdminUser.getKey(), locale)
                        + (d.getKey().getSecondKey() != null ? String.format("[%s]",
                        Optional.ofNullable(eventAdapter.getUserByID(Long.parseLong(d.getKey().getSecondKey())))
                                .map(IUser::getName)
                                .orElse(messageSourceWrapper.getMessage("discord.admin.word.show-admin-user.not-exists-user", locale)))
                        : ""),
                d.format(),
                false));
    }

    private List<AdminUser> getDynamicSettingDomains(long guildId) {
        final DynamicSettingType adminUser = DynamicSettingType.AdminUser;
        return dynamicSettingService.getList(adminUser.getKey(), guildId, AdminUser.class);
    }
}
