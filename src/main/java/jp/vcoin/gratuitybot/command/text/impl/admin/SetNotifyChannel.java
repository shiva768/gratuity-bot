...
@Component("set-notify-channel")
@Text
@Slf4j
public class SetNotifyChannel implements MessageCommand<MessageReceivedEvent> {
    ...
    @Autowired
    public SetNotifyChannel(DynamicSettingService dynamicSettingService, MessageSourceWrapper messageSourceWrapper, DiscordValidatorService discordValidatorService) {
    ...
    public void execute(MessageReceivedEventAdapter eventAdapter, Locale locale) {
        ...
        try {
            ...
            if (!validate(eventAdapter, locale, discordObject))
                return;
            dynamicSettingService.save(DynamicSettingType.NotifyChannel.getKey(), eventAdapter.getGuildId(), discordObject.getConvertedId());
